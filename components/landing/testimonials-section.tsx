    ...
    <section ref={sectionRef} className="relative py-32 lg:py-40 bg-foreground text-background overflow-hidden">
      ...
      <div className="absolute inset-0 font-mono text-[10px] text-background/[0.02] leading-tight overflow-hidden whitespace-pre select-none">
        {(() => {
          const rows = 60;
          const cols = 100;
          const seedChar = (r: number, c: number) => {
            // deterministic pseudo-random using Math.sin; stable on server and client
            const n = Math.abs(Math.sin(r * 10007 + c * 1009));
            return n > 0.7 ? '"' : ' ';
          };

          return Array.from({ length: rows }, (_, i) =>
            Array.from({ length: cols }, (_, j) => seedChar(i, j)).join("")
          ).join("\n");
        })()}
      </div>
